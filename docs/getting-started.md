...
from asymsafety.visualization.amplitude_plot import (
    plot_amplitude_vs_energy, plot_partial_wave_unitarity,
)
```
...
Three subcommands of the `asymsafety` console script:
...
# Sample the graviton-mediated scattering amplitude, with physical-consistency
# checks and the string-bootstrap comparison
asymsafety amplitude \
    --truncation eh \
    --guess g=0.7,lambda=0.14 \
    --s-range 1e-2:1e8:200 \
    --checks --compare-string \
    --output amp.npz
```
...
- Scattering amplitudes and the string-bootstrap bridge:
  `src/asymsafety/scattering/` and [`docs/scattering-amplitudes.md`](scattering-amplitudes.md);
  try the `asymsafety amplitude` CLI above.
